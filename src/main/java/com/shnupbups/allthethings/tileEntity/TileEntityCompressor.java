...
public class TileEntityCompressor extends TileEntity implements ISidedInventory,IEnergyReceiver,IMachine {
	...
	public boolean canOperate() {
		...
		if(inventory[0].stackSize<CompressingRecipes.getStackSizeToDecrease(inventory[0])) {
			return false;
		}
		if(!(storage.getEnergyStored()>energyUsePerOperate)) {
			return false;
		}
		if(inventory[1]==null&&inventory[2]==null) {
			return true;
		}
		if((inventory[1]!=null&&!inventory[1].isItemEqual(CompressingRecipes.getCompressResult(inventory[0])))||(inventory[2]!=null&&!inventory[2].isItemEqual(CompressingRecipes.getCompressResult(inventory[0])))) {
			return false;
		}
		if((inventory[1]!=null&&inventory[1].stackSize+CompressingRecipes.getCompressResult(inventory[0]).stackSize>inventory[1].getMaxStackSize())||inventory[2]!=null&&inventory[2].stackSize+CompressingRecipes.getCompressResult(inventory[0]).stackSize>inventory[2].getMaxStackSize()) {
			return false;
		}
		if(inventory[1]!=null) {
