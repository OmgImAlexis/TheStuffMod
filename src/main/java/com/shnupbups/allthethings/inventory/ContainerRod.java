...
public class ContainerRod extends Container {
	public final InventoryRod inventory;
	private static final int INV_START=3,INV_END=INV_START+26,
			HOTBAR_START=INV_END+1,HOTBAR_END=HOTBAR_START+8;

	public ContainerRod(EntityPlayer player,InventoryPlayer inventoryPlayer,InventoryRod backpack) {
		this.inventory=backpack;
		inventory.openInventory();
		this.addSlotToContainer(new SlotRod(this.inventory,0,62,30));
		this.addSlotToContainer(new SlotRod(this.inventory,1,98,30));
		this.addSlotToContainer(new SlotUpgrade(this.inventory,2,153,6));
		for(int i=0;i<3;++i) {
			for(int j=0;j<9;++j) {
	...
	public ItemStack transferStackInSlot(EntityPlayer player,int num) {
		...
		if(slot!=null&&slot.getHasStack()) {
			...
			} else if(!(itemstack1.getItem() instanceof ItemRod)) {
