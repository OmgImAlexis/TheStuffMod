	...
	public void onEntityUpdate(LivingUpdateEvent event) {
		if(event.entityLiving.isPotionActive(ModPotions.bleeding)) {
			if(event.entityLiving.worldObj.rand.nextInt(24-(MiscUtility.clamp(event.entityLiving.getActivePotionEffect(ModPotions.bleeding).getAmplifier()+1,1,20)))==0) {
				...
				// event.entityLiving.worldObj.spawnParticle("splash", event.entityLiving.posX,
				// event.entityLiving.posY, event.entityLiving.posZ, 1.0D, 1.0D, 1.0D);
			}
		}
		if(event.entityLiving.isPotionActive(ModPotions.leadPoison)) {
			if(event.entityLiving.worldObj.rand.nextInt(120)==0) {
				...
				// event.entityLiving.worldObj.spawnParticle("splash",
				// (event.entityLiving.posX+0.5F)-0.27000001072883606D,
				// (event.entityLiving.posY+0.7F)+0.2199999988079071D,
				// (event.entityLiving.posZ+0.5F), 0.0D, 0.0D, 0.0D);
			}
		}
		...
		if(event.entityLiving.isPotionActive(ModPotions.flight)&&event.entityLiving instanceof EntityPlayer&&((EntityPlayer)event.entityLiving).capabilities.isCreativeMode!=true) {
			if(event.entityLiving.getActivePotionEffect(ModPotions.flight).getDuration()<20) {
		 ...
		 * if(((EntityPlayer)event.entityLiving).capabilities.isCreativeMode != true)
		 * {((EntityPlayer)event.entityLiving).capabilities.allowFlying = false;
		 * ((EntityPlayer)event.entityLiving).capabilities.isFlying = false;}
		 * }
		 **/
